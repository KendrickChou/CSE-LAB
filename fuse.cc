        ...
        int to_set, struct fuse_file_info *fi)
{
    ...
    chfs_client::status ret1,ret2;
    struct stat st;

    ret1 = chfs->setattr(inum, attr->st_size);
    ret2 = getattr(inum, st);
    if (ret1 != chfs_client::OK || ret2 != chfs_client::OK){
        ...
        struct fuse_file_info *fi)
{
    ...
    printf("fuseserver_write: inode-%d size-%d off-%d\n", ino, size, off);
    // HOW TO SET MTIME???????????????????????????????
    ret = chfs->write(inum, size, off, buf, written_size);
    if (ret != chfs_client::OK){
        fuse_reply_err(req, ENOSYS);
        return;
    }
    
    fuse_reply_write(req, written_size);
#else
    fuse_reply_err(req, ENOSYS);
#endif
}
...
{
    printf("fuseserver_createhelper: %s\n", name);
    int ret;
...
#if 1
    chfs_client::inum parent_inum = parent;
    chfs_client::status ret;
    chfs_client::inum new_inum = 0;

    ret = chfs->mkdir(parent_inum, name, mode, new_inum);

    if(ret == chfs_client::EXIST){
        fuse_reply_err(req, EEXIST);
        return;
    } else if (ret != chfs_client::OK){
        fuse_reply_err(req, ENOENT);
        return;
    }
			 ...
			 const char *name)
{
    ...
    chfs_client::inum parent_ino = parent;
    chfs_client::inum new_ino = 0;
    printf("fuseserver_symlink : name: %s link: %s\n", name, link);

    if(chfs->symlink(parent_ino, link, name, new_ino) == chfs_client::OK){
        e.ino = new_ino;
...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
    chfs_client::inum inum = ino;
    std::string data;
    if(chfs->readlink(inum, data) != chfs_client::OK){
        fuse_reply_err(req, ENOENT);
    } else{
